...
void ModBusSendToUartTask(void);

void ModBusSendToUartTask(void){
	uint8_t buf[256]; // buffer, where we collect output data
	uint8_t count = 0; // counter for buffer fill
	for(;;)
	{
	osEvent evt = osMessageGet(ModBusOutHandle,200); // wait here 200 tick
	if (evt.status == osEventMessage)
	  {
	    buf[count++]=(uint8_t) evt.value.v;

	  }
	if (evt.status == osEventTimeout)
	  {
	    if( (count > 0) && ( count< 254) ) // ok, something in buffer exist, lets send it
	    {
	        //CDC_Transmit_FS(&buf[0], count); // by USB-CDC
	    	//HAL_UART_Transmit(&huart1, &buf[0], count, HAL_MAX_DELAY);
	    	HAL_UART_Transmit_IT_485(&huart1, &buf[0], count);
	    }
	  break;
	  count=0;
	  }
...
void ModBus_Init(void)
{
  ...
  mb_buf_in_count=0;
  mb_addr=0x00; // by default maximum possible adrress
  mb_buf_out_count=0;
    ...
    if(CRC16_IN()==0)
    {
      ...
      switch(func)
      {
        ...
        case 5:
          st=mb_buf_in[2]*256+mb_buf_in[3];
          nu=mb_buf_in[4]*256+mb_buf_in[5];
          if( (st+nu) > ModBusRegisters &&  nu > 127 && nu < 0 ) // dont ask more, that we has!
            {
              mb_buf_out[mb_buf_out_count++]=mb_addr;
              mb_buf_out[mb_buf_out_count++]=func+0x80;
              mb_buf_out[mb_buf_out_count++]=2;
          ...
          break;
        case 16: // write multiple holding registers. by bytes addr func starth startl totalh totall num_bytes regh regl ...
          st=mb_buf_in[2]*256+mb_buf_in[3];
          nu=mb_buf_in[4]*256+mb_buf_in[5];
          if( (st+nu) > ModBusRegisters) // dont ask more, that we has!
            {
